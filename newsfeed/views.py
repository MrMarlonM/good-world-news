...
def article_list(request):
    ...
    Retrieves:
    - The latest breaking news article (if any) with 'is_breaking_news' set to
    True and 'status' set to 1 (published), ordered by descending creation date
    - All other published articles (with 'is_breaking_news' set to False and
    'status' set to 1), ordered by descending creation date.

    Context:
    - `breaking_news`: The latest breaking news article or None if
    there isn't one.
    ...
    breaking_news = Article.objects.filter(
        is_breaking_news=True, status=1).order_by('-created_on')
    articles = Article.objects.filter(
        is_breaking_news=False, status=1).order_by('-created_on')
...
def article_detail(request, slug):
    ...
    Retrieves:
    - The article object based on the provided slug,
    ensuring it's published (status=1).
    - All comments associated with the article.

    Handles:
    - GET requests: Displays the article details, existing comments,
    and a comment form.
    - POST requests: Processes comment form submissions, creates a new comment,
                     associates it
                     with the article and the logged-in user, and saves it
                     to the database.
                     Then redirects back to the article detail page with
                     a success message.

    Context:
    - `article`: The Article object being viewed.
    - `comments`: A queryset of all comments associated with the article.
    - `comment_form`: An instance of the CommentForm for submitting
                      new comments.
    - `user`: The currently logged-in user.
    - `number_of_likes`: The number of likes for the article.
    - `is_moderator`: A boolean indicating whether the current user
                      is a moderator.
...
def comment_edit(request, slug, comment_id):
    ...
    - POST requests: Processes the comment edit form submission.
        - If the form is valid and the user is either the comment author
          or a moderator:
    ...
    - GET requests: (Assumed but not explicitly handled in the code)
        - Would likely render a form for editing the comment (not implemented
          in the current code).
    ...
    Returns:
    - HttpResponseRedirect: Redirects to the article detail page after
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid() and (
            comment.author == request.user or request.user.groups.filter(
            ...
            messages.add_message(
                ...
                'An error occurred. Please log in and try again.'
                )
...
def article_like(request, slug):
    ...
    article = get_object_or_404(articles, slug=slug)
    existing_like = Like.objects.filter(
        user=request.user, content_type=ContentType.objects.get_for_model(
            Article), object_id=article.id).first()
